...
pub fn train<B: AutodiffBackend>(config: TrainingConfig, device: B::Device) {
    ...
    let dataloader_train = DataLoaderBuilder::new(batcher_train)
        ...
        .build(ImageFolderDataset::skin_cancer_train());

    // NOTE: we use the test set as validation for demonstration purposes
    let dataloader_test = DataLoaderBuilder::new(batcher_valid)
        .batch_size(config.batch_size)
        .num_workers(config.num_workers)
        .build(ImageFolderDataset::skin_cancer_test());
